...
![](../images/Pasted%20image%2020260628225246.png)
## 一、为什么还要重新发明积分？
...
为什么强调“可数”，而不是任意多个集合？因为概率和积分里的极限通常来自序列：$A_1,A_2,\dots$，$f_1,f_2,\dots$。可数封闭已经足够支撑极限定理；如果强行要求对任意并都封闭，再配合所有集合都可测，反而会和合理长度的要求冲突。测度论在这里做的是一件务实的事：保留分析需要的极限操作，同时避开无法测量的病态集合。
...
简单函数可以理解成“只取有限多个值的函数”。它把复杂函数先变成有限层的阶梯：在集合 $A_i$ 上取值 $a_i$。这样积分就回到最朴素的规则：每一层的高度乘以这一层的大小，再全部相加。
...
这里要小心一个细节。对非负函数，可以先用 Tonelli 定理交换求和或积分顺序；对一般有正有负的函数，通常需要绝对可积，才能放心使用 Fubini。否则“先积哪一边”可能改变结果，甚至一边有意义、另一边发散。
...
这句话的直觉是：$\mu$ 看不见的集合，$\nu$ 也不能看见。只有这样，$\nu$ 才可能用“相对于 $\mu$ 的密度”来表示。如果 $\mu(A)=0$ 但 $\nu(A)>0$，那 $A$ 上的质量对 $\mu$ 来说没有体积，却对 $\nu$ 来说有质量，任何普通函数密度都无法把它积分出来。
...
$\mathcal G$ 可以理解成你当前能分辨的信息颗粒度。如果 $\mathcal G$ 很粗，你只能给出粗粒度预测；如果 $\mathcal G$ 很细，预测可以更接近 $X$ 本身。条件期望不是简单把 $Y$ 代进公式，而是在“只允许使用这些信息”的限制下，找一个最合适的随机变量。
