...
![](../images/Pasted%20image%2020260628174007.png)
## 一、从“参数”到“分布的空间”
...
比如同一个 Bernoulli 分布，可以用成功率 $p$ 表示，也可以用 logit 参数

$$
\theta=\log\frac{p}{1-p}
$$

表示。坐标换了，分布没有换。信息几何真正研究的是后者，也就是那一族概率分布本身，而不是某个参数表的写法。
...
这里的 $\partial_i\log p(X|\theta)$ 常叫 score。它衡量的是：参数沿第 $i$ 个方向动一点时，样本的对数似然会有多敏感。Fisher 信息矩阵可以看成这些 score 的平均协方差，所以它记录的是“模型在各个方向上有多容易被样本分辨出来”。
...
普通梯度默认每个坐标单位都一样长。自然梯度不这样看：几乎不改变分布的方向应该算短，稍微一动就明显改变分布的方向应该算长。于是它会把梯度先放回 Fisher 几何里校准，再决定怎么走。
...
$A(\eta)$ 的作用是把右边归一化成概率分布。它不只是一个修正项，还是一个凸函数；对它求梯度，会得到充分统计量的期望。也就是说，指数族把“归一化常数”“矩”和“凸几何”绑在了同一个对象上。
...
这里很容易混。最小化 $D_{\mathrm{KL}}(p\|q)$ 时，如果 $q$ 在 $p$ 有质量的地方给得太少，会被罚得很重，所以它更倾向于覆盖多个模式。最小化 $D_{\mathrm{KL}}(q\|p)$ 时，$q$ 把质量放到 $p$ 很小的地方会被罚得很重，于是它常常宁愿贴住一个高概率模式。这就是变分推断里常说的 mode-covering 和 mode-seeking。
...
所谓“直线”，要先说清楚在哪套坐标里直。自然参数中的线性插值，和混合分布中的线性插值，通常不是同一条曲线。$\alpha$-联络就是在追踪这些不同的“平直”观念。
...
## 十一、Cramér-Rao 下界：几何限制了估计精度

Fisher 信息不只是一个度量，它还给出估计精度的下界。

在正则性条件成立、且 Fisher 信息为正的标量参数情形，对无偏估计量 $\hat\theta$，Cramér-Rao 下界说：

$$
\mathrm{Var}(\hat\theta)\ge I(\theta)^{-1}
$$

多参数情形里则是协方差矩阵意义下的半正定不等式；若估计有偏、参数在边界上或模型不满足正则性，还需要使用相应的推广形式。
...
更具体一点，E 步是在当前参数下，为看不见的 $z$ 构造一个辅助分布；M 步则把这个辅助分布暂时当作权重，重新选择模型参数。它没有直接硬算原来的对数积分，而是用 Jensen 不等式给似然造出一个下界，也就是常说的 ELBO，然后交替把这个下界抬高。
...
| 领域 | 信息几何扮演的角色 |
|------|------------------|
| 统计推断 | Fisher 信息、Cramér-Rao 下界、有效估计 |
...
**K-FAC**（Kronecker-factored Approximate Curvature，Martens & Grosse 2015）通过 Kronecker 积分解近似 Fisher 矩阵的块结构，避免显式构造和求逆整个 $p\times p$ 矩阵；实际计算和存储复杂度取决于层宽、块划分、更新频率和近似方式。在部分任务中，这类二阶或近似二阶方法的收敛速度可以优于 Adam，但并非普遍保证。

### 15.2 信息几何与扩散模型

**得分匹配**（Score Matching，Hyvärinen 2005）直接最小化模型得分与数据得分之间的 Fisher 散度 $\mathbb{E}[\|\nabla_x \log p_\theta - \nabla_x \log p_\text{data}\|^2]$；在满足边界条件时可改写成不含未知数据得分的目标。它与 KL 有密切关系，但不应简单说成“自然梯度方向上的 KL 最小化”。这为扩散模型（Song & Ermon 2020）中的得分估计提供了几何解释；扩散过程本身由随机微分方程和相应的概率流共同描述，也不等同于单条 Fisher 测地线。

### 15.3 量子信息几何

Petz（1996）将经典 Fisher 度量推广到量子情形：量子 Fisher 信息（QFI）和量子相对熵定义了密度矩阵空间上的多种单调度量。对满足正则性条件的无偏估计和 $N$ 次独立样本，量子 Cramér-Rao 界写成 $\mathrm{Var}(\hat\theta)\ge 1/(N F_Q(\theta))$；多参数和测量不可兼容时还要考虑更精细的界。它给出量子态估计的精度限制，而不是保证任何实验都能达到。
