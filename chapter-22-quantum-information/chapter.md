...
这里还要区分全局相位和相对相位。$|\psi\rangle$ 和 $e^{i\phi}|\psi\rangle$ 表示同一个物理态，测量概率完全一样；但 $\alpha|0\rangle+\beta|1\rangle$ 中 $\alpha$ 和 $\beta$ 之间的相对相位会影响不同测量基下的结果。量子计算真正利用的是这种相对相位。
...
测量基可以理解成你选择了一套互斥问题。问“是不是 $|0\rangle$”和问“是不是 $|+\rangle=(|0\rangle+|1\rangle)/\sqrt2$”不是同一个问题。同一个量子态面对不同问题，会投影到不同方向，于是概率也不同。
...
维数也在这里开始爆炸。$n$ 个 qubit 的空间维数是 $2^n$，不是 $2n$。这给量子系统带来巨大表达能力，也让经典模拟迅速变得困难。纠缠正是在这个指数维空间里出现的非局部结构。
...
判断纠缠时，关键不是“相关性强不强”，而是这种相关性能否由局部状态和经典随机性解释。经典系统也能有强相关，比如两张相同的纸条分别放进两个盒子；纠缠的不同之处在于，它在多种不兼容测量基下同时表现出无法由经典共享随机变量解释的相关结构。
...
密度矩阵也让“忽略子系统”变得可计算。一个纠缠纯态的整体可以是确定的，但只看其中一部分时，要对另一部分做偏迹，得到的局部状态通常是混合态。局部不确定性可能不是因为我们制备得粗糙，而是因为信息存在于整体关联中。
...
“完全正”这个条件比“正”更强。原因是系统可能和别的系统纠缠在一起；一个物理允许的演化，不仅要把本系统的密度矩阵送到合法密度矩阵，还要在附加任意旁观系统后仍然合法。保迹则保证总概率不丢失。
...
这也解释了为什么“通用门集”重要。硬件只能实现少数基本门，但只要这些门能足够逼近任意酉变换，就能原则上实现任意量子算法。量子编译就是把理想酉变换翻译成硬件可执行的门序列。
...
**变分量子本征求解器**（VQE，Peruzzo 等 2014）和**量子近似优化算法**（QAOA，Farhi 等 2014）是噪声量子设备（NISQ 设备）上常研究的算法：用参数化量子电路表示候选态，再通过经典优化器调整参数。它们在某些问题上可能带来优势，但目前不存在对一般化学问题的无条件指数加速定理；NISQ 噪声、测量成本和优化困难都会限制实际收益。
...
稳定子码（Calderbank-Shor-Steane 1996）由 Pauli 群的对易子群（稳定子群）$\mathcal{S}$ 生成，逻辑空间是 $\mathcal{S}$ 的 $+1$ 本征空间。Knill-Laflamme 条件给出了量子纠错码精确纠正错误集 $\mathcal{E}$ 的充要条件，是算子代数与量子信息的直接桥梁。非 Abelian 任意子（如 Fibonacci 任意子）理论上支持拓扑量子门和容错方案，但其可控编织与工程实现仍是研究问题。
