...
给定样本 $S=\{x_1,\dots,x_n\}$，随机给每个样本一个独立符号：
...
Belkin 等（2019）和 Bartlett 等（2020）严格证明：在线性回归和核回归中，插值（训练误差为零）的最小范数解仍然可以泛化——条件是特征的"有效维度"足够大（信号被淹没在大量噪声特征中，最小范数解自动正则化）。
