...
Terry Lyons（1998）提出**粗糙路径理论**，彻底解决了"对不规则信号积分"的基础问题：引入**签名**（signature）——路径的迭代积分序列——作为路径的完备不变量。签名满足 shuffle 乘积律，可由有限阶截断近似，并对路径的重新参数化不变。

这带来三个深刻后果：
1. Itô 积分和 Stratonovich 积分都是粗糙路径理论的特例；Itô 和 Stratonovich 之差由二次变差（Lévy 区域）解释。
