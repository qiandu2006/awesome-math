...
这里的 $\pi:E\to M$ 可以理解成“忘掉纤维坐标，只看底点”的投影。$\pi^{-1}(p)$ 就是点 $p$ 上方附着的纤维。局部平凡化说的是，在一个小邻域里你可以给所有纤维统一编号；但换到另一个邻域，这套编号可能要重新旋转、翻转或做群作用。
...
为了描述纤维丛，我们用开覆盖 $\lbrace U_i\rbrace$。
...
cocycle 条件的直觉是：如果从第 $i$ 套坐标换到第 $j$ 套，再换到第 $k$ 套，最后回到第 $i$ 套，整体不应该凭空产生矛盾。转移函数记录扭曲，cocycle 条件保证这些扭曲能一致地粘成一个全局丛。
...
没有联络时，“不同纤维里的向量是否相同”没有意义。联络不是额外装饰，而是给出了比较规则。曲率则说明这个比较规则是否路径无关：若绕小圈回来发生变化，说明局部几何里有弯曲或场强。
...
“全局非零截面”可以想成在每个点连续选一个非零向量。若做不到，就说明丛有不可消除的扭曲。示性类把这种做不到的障碍变成上同调类，因此它不是只给丛贴标签，而是在测量全局选择失败的原因。
...
主丛和普通向量丛的区别是：纤维本身不是“向量”，而是一组参考标架或规范选择。群 $G$ 作用在这些标架上，表示同一个几何对象的不同描述。规范变换就是在每个点换一套标架。
