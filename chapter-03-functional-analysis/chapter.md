...
这个极限函数有跳跃，不在 $C([0,1])$ 中。但在 $L^2$ 范数下，过渡区间的面积越来越小，所以 $\{f_n\}$ 是 Cauchy 列。也就是说：$C([0,1])$ 配上 $L^2$ 范数并不完备，它的完备化是 $L^2([0,1])$。
...
> **傅里叶级数就是在做正交投影**：$H = L^2([0, 2\pi])$，$M_n = \text{span}\{1, \cos x, \sin x, \dots, \cos nx, \sin nx\}$。第 $n$ 个部分和就是 $f$ 到 $M_n$ 的正交投影。投影定理保证了它是最优的 L² 逼近。
...
注意 Riesz 定理需要 Hilbert 空间（即内积结构）。如果你只有 Banach 空间（比如 $C([0,1])$ 配 sup 范数），事情就不一样。$C([0,1])$ 上的线性泛函 $\delta_0(f) = f(0)$ 不能通过某个连续函数 $g$ 写成 $\int f(x)g(x)dx$——因为没有连续函数能在除了 0 以外的点处处为 0，又在 0 处"集中"出有限质量。$\delta$ 更自然地是一个**测度**。这正是 Riesz-Markov-Kakutani 表示定理的结论：$C([a,b])^* \cong \{\text{有界 Borel 测度}\}$。
...
在连续小波变换中，基底 $\{\psi_{a,b}\}$ 是**过度完备**的——$a$ 和 $b$ 是连续参数，基底的"个数"远多于必要。这意味着表示不是唯一的：同一个信号有（连续）无穷多种小波系数表示。
...
两者都是 $L^2(\mathbb{R})$ 上的**基展开**。差别在于基底的几何性质：

- **傅里叶基底** $\{e^{i\omega t}\}$：标准正交基，完全对角化了卷积算子（平移群的作用），但在时间定位上完全无能为力。
- **小波基底** $\{\psi_{a,b}\}$：不一定是标准正交基（CWT 是框架，DWT 可以是标准正交基），由**缩放+平移**的仿射群作用生成。它在时间和频率上都有局部性。
