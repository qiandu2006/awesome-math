...
- **傅里叶基底** $\lbrace e^{i\omega t}\rbrace$：标准正交基，完全对角化了卷积算子（平移群的作用），但在时间定位上完全无能为力。
- **小波基底** $\lbrace \psi_{a,b}\rbrace$：不一定是标准正交基（CWT 是框架，DWT 可以是标准正交基），由**缩放+平移**的仿射群作用生成。它在时间和频率上都有局部性。
