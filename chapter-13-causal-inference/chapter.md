...
Chernozhukov 等（2018）解决了“用高维 ML 估计低维因果参数”时的一类正则化偏差问题：将目标参数（如平均处理效应 ATE）的估计写成 Neyman 正交化矩方程，使用交叉拟合的机器学习方法估计干扰函数（倾向得分、条件均值），再用正交残差减弱 ML 误差的一阶影响。在重叠性、矩条件和 nuisance 估计速率等假设下，可以得到渐近正态或 $\sqrt{n}$-量级结果；并非任意 ML 方法、任意数据分布都自动满足这些结论。
