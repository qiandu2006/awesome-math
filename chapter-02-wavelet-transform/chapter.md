...
对这个信号做傅里叶变换，频谱幅度图上会出现 1Hz 和 3Hz 两个峰。但从这种全局频谱里，很难直接判断这两个频率**分别在什么时候出现**——是先有 1Hz 再有 3Hz，还是两者同时存在，还是交替出现？完整 Fourier 变换仍包含足以重建信号的幅度和相位信息；问题在于它没有把“某个频率发生在某个时刻”直接表示成局部坐标。

这不是傅里叶变换的缺陷，而是表示方式的选择：**Fourier 原子是无限延伸的正弦波**，每个频率系数都由整个时间轴上的信号共同决定。于是时间位置没有被显式局部化；若只看幅度谱，平移等信息还会进一步丢失。

这里需要先区分两个经常被混在一起的说法：

- **只看幅度谱**：这是最常见的“频率图”。它告诉我们每个频率含有多少能量，但完全看不出能量发生在什么时间。
- **保留完整的复数傅里叶变换**：幅度和相位都保留时，原则上可以通过逆变换精确重建原信号，时间信息并没有从数学上消失。只是这些时间信息被编码在所有频率的相位关系里，而不是直接以一个“时间坐标”出现。

一个时间平移就能把这个区别说明白。设 $g(t)$ 是一小段只在局部出现的振荡，令

$$
f_1(t)=g(t-t_1),\qquad f_2(t)=g(t-t_2).
$$

它们的傅里叶变换满足

$$
\hat f_2(\omega)=e^{-i\omega(t_2-t_1)}\hat f_1(\omega),
$$

因此

$$
|\hat f_2(\omega)|=|\hat f_1(\omega)|.
$$

也就是说，把同一段 100Hz 振荡从第 1 秒移动到第 5 秒，幅度谱完全不变；幅度谱只能说“有 100Hz”，不能说“100Hz 在第几秒发生”。完整变换中的相位确实发生了变化，但要从相位中读出多个局部事件的先后和位置并不直接。

因此，“只有频率不够”并不是说傅里叶变换不能重建信号，而是说：**当我们关心非平稳信号的局部事件时，只有一个频率轴的表示不方便直接回答问题。** 现在回到上面的“傅里叶 vs 小波”图，按“左下 → 左上 → 右下 → 右上”的顺序再看一遍。

**第一步：先看左下角的原始信号。** 横轴是时间。$t<4$ 时波形比较稀疏，每秒振荡 1 次；$t>4$ 时波形突然变密，每秒振荡 3 次。只用眼睛就能看出“频率在 $t=4$ 发生了变化”。这是因为时域图保留了时间位置，但如果波形中混合了许多频率或带有噪声，靠数波峰就很难判断每种频率有多少。

**第二步：再看左上角的 Fourier 基底。** 图中的每条正弦波都从横轴左端延伸到右端。做傅里叶变换，可以想象成拿这些不同疏密的正弦波，与左下角的**整段信号**逐一匹配。某个频率得到一个系数，这个系数汇总了该频率在整个观察区间内的匹配程度；它没有附带“这是在 $t<4$ 匹配到的”或“这是在 $t>4$ 匹配到的”这样的标签。

**第三步：匹配结果就是右下角的幅度谱。** 横轴已经从“时间”换成了“频率”，所以我们能清楚看到 1Hz 和 3Hz 两处峰值，知道整段信号中含有这两个主要频率。但图中已经没有时间轴：仅看这张频谱，无法判断是先出现 1Hz 再出现 3Hz，还是顺序相反，甚至无法判断它们是否一直同时存在。这就是这里所说的“只有频率”。

**第四步：最后看右上角的小波基底。** 三条曲线是同一个局部小波分别移动到 $t=-2$、$t=0$ 和 $t=2$ 的结果。与延伸到整个时间轴的正弦波不同，每个小波只检查信号的一小段。因此匹配系数天然带有一个位置标签：在 $t=2$ 附近匹配到的结果，不会和 $t=-2$ 附近混在一起。图中展示的是“平移”；如果再把小波压窄或拉宽，就能分别匹配高频细节和低频变化。这样，一个小波系数就同时回答了两个问题：

> **多宽的小波匹配得最好？** 这对应频率或尺度；**小波放在哪里时匹配得最好？** 这对应时间或位置。

现在整个动机就连起来了：**时域图能看见“何时变化”，全局频谱能看见“有哪些频率”，而我们想要一种同时保留两者的表示。** 最直接的办法是在每个时间位置截取一小段做傅里叶变换，这就是下一节的短时傅里叶变换（STFT）；小波则进一步让这段窗口能够随尺度变宽或变窄。
...
| 表示 | 能直接读出的信息 | 典型问题 |
|------|------------------|----------|
| 原始时域信号 | 某个时刻的幅度 | 看得见变化，但不容易分辨频率 |
| 全局傅里叶幅度谱 | 整段信号包含哪些频率 | 看得见频率，但看不见它们出现的时间 |
| 时频/小波表示 | 某个时间（位置）上的某个频率/尺度 | 可以追踪局部变化和突发事件 |

所以本节标题中的“只有频率”，更准确地说是“**只有全局的频率统计，没有显式的时间局部坐标**”。这正是我们接下来引入滑动窗口和局部小波的理由。
...
紧支撑意味着小波函数在有限区间外恒为零。在离散滤波器组实现中，这对应有限长度滤波器，因此单步卷积只需有限次运算；边界处理和有限精度运算仍会带来实现误差。Daubechies 小波是离散小波变换中常用的一族基底，JPEG2000 等标准采用的也是相关的双正交小波构造。

Daubechies dbN 系列中，N 是**消失矩**的数目：

> 如果一个小波有 $p$ 个消失矩，即 $\int t^k \psi(t)dt=0$ 对 $k=0,\dots,p-1$ 成立，那么它与次数低于 $p$ 的多项式正交。因而在远离边界、并满足相应离散化条件时，平滑的局部多项式趋势会产生很小或为零的细节系数，突变和纹理则更容易留下明显系数。

这揭示了小波设计中的典型权衡：在正交性、紧支撑、对称性、消失矩和光滑性之间，通常不能同时把每一项都做到最好。Daubechies 小波给出了其中一类重要构造。
