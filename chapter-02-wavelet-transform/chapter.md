...
Fourier 变换给每个频率 $\omega$ 分配一个系数

$$
\widehat f(\omega)=\int_{\mathbb R}f(t)e^{-i\omega t}\mkern3mu dt.
$$

这里的核函数 $e^{-i\omega t}$ 延伸在整条时间轴上，积分也汇总了所有时刻的贡献。因此，$\widehat f(\omega)$ 直接回答的是“整个信号与频率 $\omega$ 的振荡有多匹配”，而不是“频率 $\omega$ 在什么时刻出现”。对图中的信号，频谱可以显示 1 Hz 和 3 Hz 两种成分，却没有一个时间变量用来直接标明它们的先后顺序。

这并不意味着 Fourier 变换丢失了时间信息。完整的 Fourier 变换同时保留幅度和相位，原则上可以通过逆变换重建原信号。它的局限在于，变换结果 $\widehat f(\omega)$ 只以频率 $\omega$ 为索引，并没有一个表示时刻的变量。某个事件何时发生，不会单独记录在某个 Fourier 系数中，而是分散在不同频率系数之间的相位关系里。因此，要确定事件的位置，通常需要综合整个频谱，甚至先通过逆变换还原信号；无法像读取一个时间坐标那样，直接从单个系数中得到答案。下面用最简单的平移例子说明这一点。

设 $g\in L^1(\mathbb R)\cap L^2(\mathbb R)$ 表示一个局部事件，并把它移动到时刻 $\tau$：

$$
f_\tau(t)=g(t-\tau).
$$

在 Fourier 变换的定义中令 $u=t-\tau$，便得到

$$
\begin{aligned}
\widehat f_\tau(\omega)
&=\int_{\mathbb R}g(t-\tau)e^{-i\omega t}\mkern3mu dt\\
&=e^{-i\omega\tau}\int_{\mathbb R}g(u)e^{-i\omega u}\mkern3mu du\\
&=e^{-i\omega\tau}\widehat g(\omega).
\end{aligned}
$$

这个公式把“移动事件”在频域中的影响说得很清楚：波形的频谱 $\widehat g(\omega)$ 没有改变，出现时刻 $\tau$ 只产生了随 $\omega$ 线性变化的相位因子 $e^{-i\omega\tau}$。由于该因子的模恒为 1，

$$
|\widehat f_\tau(\omega)|=|\widehat g(\omega)|.
$$

所以，同一个事件无论出现在什么时刻，幅度谱都完全相同。若保留完整的复数频谱，当然仍能从相位关系中恢复它的位置；但要做到这一点，必须联合解释所有频率上的相位，而不能在某个系数旁直接读出一个时间坐标。

多个局部事件同时存在时，这种间接性更加明显。以有限个事件为例，若

$$
f(t)=\sum_{k=1}^N g_k(t-t_k),
$$

那么

$$
\widehat f(\omega)=\sum_{k=1}^N e^{-i\omega t_k}\widehat g_k(\omega).
$$

在每个频率上，不同事件的贡献先叠加成一个复数；事件内容 $\widehat g_k$ 与出现时刻 $t_k$ 通过相位因子混合在一起。最终得到的坐标仍然只有 $\omega$，并没有为每个局部事件保留一个可以逐项对应的“时刻—频率”标签。这正是全局 Fourier 表示对非平稳信号“不够直接”的含义：信息并未丢失，但局部结构没有被显式组织出来。

于是，问题自然变成：能否在分析信号时就引入位置参数，使每个系数同时对应“哪里”和“什么尺度”，而不必事后从全局相位中反推？更准确地说：

> 给定 $f\in L^2(\mathbb R)$，能否构造带有位置和尺度参数的系数，使局部事件易于定位，同时仍能控制能量并重建 $f$？

下一节先考察最直接的办法：给 Fourier 分析加上一个可移动的时间窗口。随后再看固定窗口为何仍有限制，以及小波如何把“位置”和“尺度”共同变成显式坐标。后文以能量恒等式和重建公式作为问题是否真正解决的标准。这里也不主张小波在所有任务上都优于 Fourier 方法；对于平稳信号、卷积算子和精确频率估计，全局频率坐标往往仍然更合适。
