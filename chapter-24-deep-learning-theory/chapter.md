...
![](../images/Pasted%20image%2020260628173818.png)
## 一、为什么最后来到深度学习？
...
“学习坐标系”这句话可以更具体地理解：原始像素、词元或传感器读数并不一定是适合任务的坐标。网络的中间层把数据搬到新的表示空间，在那里相同类别更靠近，不同类别更容易分开，任务相关因素被放大，干扰因素被压低。
...
高维非凸地形也和低维直觉不同。低维里坏局部极小像明显的坑；高维里更多困难来自鞍点、病态曲率和很平的方向。过参数化让低损失解形成大范围连通区域，一阶方法往往只需要找到其中一个泛化还不错的点。
...
直觉上，NTK 把“改变参数会怎样改变输出”固定成一个相似度函数。如果两个输入的梯度方向很接近，更新参数时它们的输出会一起变化；如果梯度方向几乎正交，它们就比较独立。宽网络理论把这种训练动态冻结下来，转化成核方法。
...
分母里的 $\sqrt d$ 是为了控制尺度。如果 $Q$ 和 $K$ 的维度很高，点积的方差会随 $d$ 增大，softmax 容易过早饱和，梯度变小。除以 $\sqrt d$ 后，相似度落在更稳定的范围里。
...
所以 grokking 不是单纯“训练久一点就好”。它依赖任务结构、数据规模、正则化和优化路径。它有价值的地方，是把记忆解和算法解之间的竞争显露出来，让我们看到泛化可能在训练损失已经很低之后才发生。
...
这也解释了为什么单个神经元未必对应单个概念。高维空间可以用方向来表示特征，多个特征还可能叠加在同一组神经元上。机制可解释性很多时候不是“给每个神经元命名”，而是寻找激活空间里的功能方向和回路。
...
**链式思维推理**（Chain-of-Thought，Wei 等 2022）通过提示模型生成中间步骤，在许多复杂推理任务上能提升准确率。从计算角度看，生成更多 token 会增加推理时间和可用的串行计算步数；但把这种现象直接表述为“从常数级提升至多项式级”或给出单一电路复杂度归类，需要额外的模型、精度和计算模型假设，不能当作一般定理。
...
在常见的 KL 正则化奖励最大化模型中，最优策略具有相对于参考策略按指数倾斜的形式；RLHF 的实际训练还包含奖励模型误差、采样和优化近似，不能简单等同于 Bradley-Terry 模型的闭式解。**直接偏好优化**（DPO，Rafailov 等 2023）在特定偏好模型假设下绕过显式奖励模型，将偏好对比优化写成监督学习目标。
