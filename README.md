...
项目采用统一的 [写作规范](WRITING_GUIDE.md)，新章节可以从 [章节模板](chapter-template.md) 开始。核心要求是：

- 先把问题、研究对象、假设与适用范围定义清楚，再引入抽象概念。
- 定义写明对象类型、量词和全部条件；定理完整陈述假设与结论。
- 每章选择 2–5 个支撑主线的核心结果，并给出可逐步检查的完整证明。
- 关键推导不跳步；交换极限、积分、求和或微分时说明合法性依据。
- 为核心概念配置可计算的例子、反例与边界情形，并说明重要假设不可删除的原因。
- 明确区分严格论证、直觉解释、形式推导和数值证据。
- 统一符号、链接与引用，让每章都能还原“问题—定义—定理—证明—应用”的推理链。
