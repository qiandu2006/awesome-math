...
$$
H^k = \ker(d:C^k\to C^{k+1}) / \mathrm{im}(d:C^{k-1}\to C^k)
$$
...
$$
H^k_{\mathrm{dR}}(M)
= \frac{\lbrace \text{闭 }k\text{-形式}\rbrace}{\lbrace \text{恰当 }k\text{-形式}\rbrace}
= \frac{\ker d}{\mathrm{im} d}
$$
...
2. 用链群把几何对象变成向量空间。
3. 用边界算子表达"边界"。
4. 用 $\ker\partial/\mathrm{im}\partial$ 测量闭合但未被填充的结构。
