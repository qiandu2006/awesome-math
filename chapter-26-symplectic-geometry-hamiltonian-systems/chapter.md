...
按上面的符号约定，$\lbrace F,G\rbrace=dF(X_G)$，也就是 **$F$ 沿着 $G$ 生成的流的瞬时变化率**。等价地，$G$ 沿着 $F$ 的流变化率是 $\lbrace G,F\rbrace$。

最基本的检验是 $\lbrace q,p\rbrace=1$。对谐振子 $H=(p^2+q^2)/2$，有 $\lbrace q,H\rbrace=p$、$\lbrace p,H\rbrace=-q$，这正好重新得到上一节的两条运动方程。
