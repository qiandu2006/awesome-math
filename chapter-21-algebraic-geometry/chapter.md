...
$$
V(I)=\lbrace x\mid f(x)=0,\ \forall f\in I\rbrace
$$
...
它先给单项式排一个顺序，然后把多项式除法推广到多变量情形。若一组生成元 $G=\lbrace g_1,\dots,g_m\rbrace$ 的首项理想足够好，那么任意多项式对 $G$ 做除法，余数就成为标准形。
...
Candelas 等（1991）发现，某些 Calabi-Yau 3-流形（弦论的紧化空间）成对出现，其上的 A 模型（辛几何，量子上同调）和 B 模型（复几何，Hodge 理论）彼此交换——这就是**镜像对称**。它预言了许多枚举几何关系（如有理曲线计数公式）；这些关系已在若干具体模型和维数中得到证明，但不应理解为对所有 Calabi-Yau 的同一套无条件定理。

同调镜像对称（Kontsevich 1994）进一步预言，在满足适当几何、紧性和系数条件时，Fukaya $A_\infty$-范畴与导出相干层范畴之间存在等价。这是一个由许多具体定理逐步实现的研究纲领，而不是已经对所有情形完成的统一证明。
