...
![](../images/Pasted%20image%2020260628171346.png)
## 一、为什么“有公式”还不够？
...
Halko、Martinsson 与 Tropp（2011）将随机化引入数值线性代数：用随机投影将大矩阵压缩至低维子空间，再在小矩阵上做 SVD，得到近似低秩分解。具体复杂度取决于矩阵形状、目标秩、过采样和是否需要多次幂迭代，不能统一写成 $O(mn\log k)$；它通常能显著减少大矩阵的计算和存储成本。这一思路（随机化 SVD、Randomized LU、Sketching）已被许多数值和机器学习库采用。
