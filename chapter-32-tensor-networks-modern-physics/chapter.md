...
- 纠缠熵满足 Ryu-Takayanagi 公式：CFT 子系统的纠缠熵 = AdS 中极小曲面的面积（以 $4G_N$ 为单位）。
...
量子化学中的电子结构问题是求解多电子 Schrödinger 方程，指数复杂度使精确求解不可行。张量网络方法（DMRG 量子化学版本，Chan & Head-Gordon 2002）通过将活性轨道按相关性排序并压缩 MPS 键维，在强相关分子（金属酶活性位点、过渡态、高自旋态）上达到完全活性空间（CASSCF）精度，同时将计算代价从指数级降至多项式级。这是当前量子计算机（VQE）的强劲经典竞争对手。
